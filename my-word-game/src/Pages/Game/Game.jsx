...
export default function Game() {
  ...
  const [gameTime, setGameTime] = useState(8 * 60); // 8 Minutes
  const [wordTime, setWordTime] = useState(8);     // 8 Seconds
  ...
  useEffect(() => {
    const fetchWords = async () => {
      setLoading(true);
      try {
        // 1. Increase 'max' to 400 to get a bigger pool to filter from
        const response = await fetch(`https://api.datamuse.com/words?sp=??????&md=f&max=400`);
  ...
  const triggerNextWord = () => {
    setCurrentIndex((prev) => {
      if (prev < words.length - 1) {
        timeRef.current = 8; // Reset the Ref
        setWordTime(8);      // Reset the UI
        return prev + 1;
      } else {
        setIsGameOver(true);
        return prev;
  ...
  const submitToAi = async () => {
    ...
    try {
      ...
      // Sometimes Gemini adds "json" or backticks, this regex extracts ONLY the JSON part
      ...
      console.log("Success! Navigating with data:", data);
  ...
  return (
    <div className="min-h-screen bg-indigo-950 text-white p-6 flex flex-col items-center">
      ...
      <div className="w-full max-w-5xl flex justify-between items-center mb-10 bg-white/5 p-4 rounded-2xl backdrop-blur-sm border border-white/10">
        ...
        <div className="text-xl font-mono">Word: {currentIndex + 1} / 60</div>
      </div>
        ...
        <div className="flex flex-col items-center animate-in fade-in zoom-in duration-500 w-full">
          <div className="bg-white text-indigo-900 rounded-[3rem] p-20 shadow-2xl text-center w-full max-w-2xl mb-8">
            ...
            <div className="relative w-full bg-gray-100 h-4 rounded-full overflow-hidden border-2 border-gray-50">
              <div
                className={`h-full transition-all duration-1000 ease-linear ${wordTime < 4 ? 'bg-red-500' : 'bg-indigo-600'}`}
                style={{ width: `${(wordTime / 8) * 100}%` }}
              />
            </div>
